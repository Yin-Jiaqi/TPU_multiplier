...
low=0;high=2**16-1;
data={}
for aaaa in [[2],[4],[8],[2,4],[4,8],[2,4,8]]:
    input1=np.random.randint(low, high, size=[128,128,128], dtype=int)
    input2=np.random.randint(low, high, size=[128,128,128], dtype=int)
    model_class=uint_k_base(tuple(input1.shape[1:]),tuple(input2.shape[1:]),aaaa)
    opt,ttt,t1,t2=model_class.call(input1,input2,0,0)
    data[tuple(aaaa)]={}
    data[tuple(aaaa)]['mse']=np.mean((opt-np.matmul(input1,input2))**2)
    data[tuple(aaaa)]['pe']=np.mean((opt-np.matmul(input1,input2))/np.matmul(input1,input2))
    data[tuple(aaaa)]['time1']=ttt
    data[tuple(aaaa)]['time2']=t1
    data[tuple(aaaa)]['time3']=t2
